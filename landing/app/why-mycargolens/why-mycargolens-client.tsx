...
/** Exit-only curve for the "legacy mess collapses" beat — the clutter
 * accelerates offstage. Deliberate: the ease-in ban applies to entrances
 * and UI responses, not to content leaving the viewer's attention. */
...
const DIFFERENTIATORS = [
  ...
  },
  {
    ...
    body:
      "ISF-10/ISF-5, Entry Summary 7501, Cargo Release 3461, Type 86 de-minimis entries, container tracking, manifest queries, and HTS classification all live in one quiet inbox. Work a ranked queue instead of juggling a dozen browser tabs. Filing customs should feel like clearing email, not operating legacy software.",
    hover: "lift" as const,
  },
  {
    icon: Tag,
  ...
  },
  {
    ...
    body:
      "Self-filing importers get a guided, AI-checked path to filing their own ISF and Entry. Brokers and 3PLs get the same rails with team accounts, role-based access, and a public API for filing programmatically. Same data, same plumbing — surfaced for how you actually work.",
    hover: "lift" as const,
  },
];
 ...
 * Marketing beat (once, on scroll); MotionConfig reducedMotion="user" strips
 * the y-transform for reduced-motion users, leaving a plain fade. */
    ...
    <>
      ...
      >
        <ul className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {DIFFERENTIATORS.map(({ icon: Icon, title, body, hover }, idx) => (
            <motion.li key={title} {...cardReveal(idx)} className="rounded-2xl border border-border/60 bg-card p-6">
      ...
      >
        <ul className="grid gap-4 md:grid-cols-3">
          {VALUE_MATH.map((row, idx) => (
            <motion.li key={row.label} {...cardReveal(idx)} className="rounded-2xl border border-border/60 bg-card p-6">
      ...
      >
        <ol className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {ONBOARDING.map((s, idx) => (
            <motion.li key={s.step} {...cardReveal(idx)} className="rounded-2xl border border-border/60 bg-card p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-6">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              ...
              access. Prefer your own stack? Public API v1 gives you scoped keys, ISF and
              entry read/write, XML upload, and webhooks. The calm inbox and AI pre-flight
              scale from your first client to your hundredth — no per-seat enterprise
